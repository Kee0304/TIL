...
#         ari_list.append(char)
# 
# a=inp.replace('(','z').replace(')','z').replace('+','z').replace('-','z').replace('*','z').replace('/','z').split('z')
# 
# ari_idx=0
# for i in range(len(a)):
#     if a[i]=='':
#         a[i]=ari_list[ari_idx]
#         ari_idx+=1
import sys
        ...
        if token.isdigit():                                                 # 숫자면
            ...
            if not stack:                                                   # 스택이 비어있으면
                ...
                if token == "(":                                            # 여는 괄호면
                    ...
                    while stack and stack[-1] != "(":                       # 스택이 차있고 top이 여는 괄호가 아닐 때까지
